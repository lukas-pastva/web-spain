...
function CombinedDaylightVideos() {
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch('/api/videos/combined-daylight');
  ...
  const regenerate = async () => {
    setGenerating(true);
    try {
      await fetch('/api/videos/generate/combined-daylight', { method: 'POST' });
      fetchQueueStatus();
    } catch (err) {
  ...
  return (
    <div className="video-list-page">
      <div className="page-header">
        <h1 className="page-title">Combined Daylight Videos</h1>
      ...
      <p className="page-description">
        All daylight videos (sunrise to sunset only) concatenated into one continuous time-lapse.
      </p>

      {videos.length === 0 ? (
        <div className="empty-state card">
          <p>No combined daylight video generated yet.</p>
          <p>Generate daylight videos first, then create the combined video.</p>
...
export default CombinedDaylightVideos;
